...
public class SQLiteUserRepository implements UserRepository {
    ...
    public Optional<User> findByName(String name) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            String sql="select EMailAdresse, 'KUNDE' FROM Kunde\n" +
                    "WHERE EMailAdresse=?\n" +
                    "UNION\n" +
                    "SELECT EMailAdresse, 'PROJEKTLEITER' FROM Projektleiter\n" +
                    "WHERE EMailAdresse=?" ;
    ...
    public long countByNameAndPassword(String name, String password) {
        try (Connection connection = dataSource.getConnection()) {
            ...
            String sql = "SELECT count(*)  FROM  Nutzer where EMailAdresse=? and Password=? ;";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.closeOnCompletion();
            preparedStatement.setObject(1, name);
